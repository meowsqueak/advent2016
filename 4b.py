input_data = """fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
def room_is_real(name, checksum):
    ...
    # sort values
    sorted_x = sorted(freq.items(), key=lambda x: (1./x[1], x[0]))
    cs = "".join((x[0] for x in sorted_x[:5]))
    return checksum == cs

def decrypt(name, iterations):
    result = ""
    for letter in name:
        if letter == '-':
            letter = " "
        else:
            letter = chr(ord('a') + ((ord(letter) - ord('a') + iterations) % 26))
        result += letter
    return result

sum = 0
for room in input_data.split("\n"):
    name, sector_id, checksum = re.match(r'(.*)-(\d+)\[(.....)\]', room).groups()
    if room_is_real(name, checksum):
        if decrypt(name, int(sector_id)) == "northpole object storage":
            print(sector_id)
        sum += int(sector_id)
